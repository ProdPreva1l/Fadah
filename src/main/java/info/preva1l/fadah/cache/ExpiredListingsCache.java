...
public class ExpiredListingsCache {
    ...
    public boolean playerHasExpiredListings(UUID uuid) {
       return !Fadah.getINSTANCE().getDatabase().getExpiredItems(uuid).join().isEmpty();
    }
    public boolean playerHasCachedExpiredListings(UUID uuid) {
        return !expiredListings.containsKey(uuid);
    }
}
